...
const createTouchTexture = (): TouchTextureResult => {
  const size = 64;
...
const SHAPE_MAP: { [key: string]: number } = {
  square: 0,
  circle: 1,
  triangle: 2,
  diamond: 3,
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      if (threeRef.current) {
        ...
        if (t.targetElement && t.onPointerDown && t.onPointerMove) {
          t.targetElement.removeEventListener("pointerdown", t.onPointerDown as any);
          t.targetElement.removeEventListener("pointermove", t.onPointerMove as any);
        }
      ...
      let touch: TouchTextureResult | undefined;
      ...
      const targetElement = container.closest("section") || window;
      targetElement.addEventListener("pointerdown", onPointerDown as any, {
        passive: true,
      });
      targetElement.addEventListener("pointermove", onPointerMove as any, {
        passive: true,
      });
      let raf = 0;
      const animate = () => {

...
### 1. The Math: Constant Product ($x \cdot y = k$)
...
1. They deposit a quantity $\Delta x$ into the contract.
2. The reserve of Token A becomes $x + \Delta x$.
3. To keep $k$ constant, the contract computes the new reserve of Token B ($y - \Delta y$) such that:
   $$(x + \Delta x) \cdot (y - \Delta y) = k$$
4. Rearranging the formula, the trader receives $\Delta y$ tokens:
